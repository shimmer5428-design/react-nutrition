...
type Tab = 'builtin' | 'custom' | 'history' | 'family'
...
export default function AddFoodPanel({
  ...
  // History: collect unique foods from this person's other days this week
  const historyFoods: FoodItem[] = []
  const seenHistory = new Set<string>()
  for (const dayMeals of Object.values(personWeekMeals)) {
      ...
      {tab === 'history' && (
        <div className="add-food-tab-content">
          {historyFoods.length === 0 ? (
            <p className="meal-empty">本週尚無其他天的食材紀錄。</p>
          ) : (
